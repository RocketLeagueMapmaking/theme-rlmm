        ...
        <template #page-header>
            <div class="layout-rlmm-theme-wrapper">
                <div class="changelog-page">
                    ...
                    <div v-for="item in fetchedItems" :key="item.id">
                        <h2>{{ item.name }} <Badge v-if="item.prerelease" text="Pre-release" type="warning" /></h2>
                        <p v-if="item.published_at">Published on {{ new Date(item.published_at).toLocaleString() }}</p>
                        <a v-if="item.html_url" :href="item.html_url">View release <OutboundLink /></a>

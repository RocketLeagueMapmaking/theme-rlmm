    ...
    <div class="navbar-notification layout-rlmm-theme-wrapper page-sidebar" v-if="show">
        ...
        <div class="page-sidebar-headers" v-if="headers.length > 0">
            <div class="page-sidebar-header" v-for="header in headers" :key="header.slug">
                <a :href="'#' + header.slug" @click="activeHeader = header.slug"
                    :class="{ isActive: activeHeader === header.slug }"
                    :style="{ 'margin-left': Math.max(0, (header.level - 2)) * 12 + 'px' }">{{ header.title }}</a>
            </div>
        </div>

        <div class="page-sidebar-item" v-for="item in ($page.frontmatter.items || [])" :key="item.text"
        ...
        <div class="page-sidebar-actions" v-for="item in ($page.frontmatter.actions || [])" :key="item.text">
            <Banner :iconEnabled="false" :icon="item.icon" :type="item.type || 'info'"
                :backgroundHoverColor="`var(--c-${item.type || 'info'}-bg)`" textColor="var(--c-text)"
                v-if="item.banner == undefined || item.banner">
                <ExternalLink :href="item.link" :anchor="item.external" :useAccentColor="false">{{ item.text }}
                </ExternalLink>
            </Banner>
    ...
    computed: {
        ...
        show: function () {
            return this.enabled
                ...
                && this.$frontmatter.hideSidemenu !== true
                && this.windowWidth >= this.minWidthSidemenu
                && (this.$page.headers || this.$page.frontmatter.items || this.$page.frontmatter.actions)
        },
...
.page-sidebar-header a:hover,
.page-sidebar-header a.isActive {
    color: var(--c-text);

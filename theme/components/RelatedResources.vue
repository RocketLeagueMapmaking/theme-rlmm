<template>
    <div class="related-resources" v-if="visible">
        <h3>{{ title }}</h3>
        <v-row>
            <v-col v-for="(page, i) in pages" :key="i" cols="12" md="4">
                <router-link v-if="page" :to="page.path">
                    <v-card class="d-flex align-center justify-center rounded" height="80px">
                        <Icon v-if="page && page.icon" :icon="page.icon" style="width: 2.5em; height: 2em;"
                            color="var(--c-brand)" class="ml-4"></Icon>
                ...
                <v-card v-else class="d-flex align-center" height="80px">
                </v-card>
            </v-col>
        </v-row>
    </div>

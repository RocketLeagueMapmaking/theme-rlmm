<template>
    <header class="navbar" :class="{ 'notification-display': notification != null }">
        <NotificationBanner v-if="notification" :message="notification.message" :storageKey="notification.storageKey"
            :dismisseable="notification.dismisseable" :pages="notification.pages" :color="notification.color"
            :textColor="notification.textColor" :startDate="notification.startDate" :endDate="notification.endDate"
            :once="notification.once" />
        <SidebarButton @toggle-sidebar="$emit('toggle-sidebar')" />

        <RouterLink :to="$localePath" class="home-link">
            <img v-if="navbarData.logo" class="logo" :src="$withBase(navbarData.logo)" :alt="navbarData.title" />
            <span v-if="navbarData.title" ref="siteName" class="site-name" :class="{ 'can-hide': navbarData.logo }">{{
                navbarData.title }}
            </span>
        </RouterLink>
...
export default {
    ...
    computed: {
        ...
        /** @returns {{ icon: string | undefined, title: string }} */
        navbarData() {
            return {
                title: this.navbarConfig.title || this.$siteTitle,
                icon: this.navbarConfig.icon || this.$site.themeConfig.logo,
            };
        },
    ...
    methods: {
        setActiveNotification(notifications, pages, $page, dismissedKey) {
            const dismissedNotifications =
                themeEvents.getLocalStorageItem("notifications-dismissed")
                    .value || [];
            ...
            const active = notifications
                .filter((n) =>
                    isNotificationEnabled(
                        ...
                        pages,
                        $page,
                        dismissedNotifications.includes(n.storageKey) ||
                        (dismissedKey
                            ? dismissedKey === n.storageKey
                            : false)
                    )
                )
        ...
        fetchNotifications(dismissedKey = null) {
            ...
            if (notifications.length > 0) {
                return this.setActiveNotification(
                    notifications,
                    this.$site.pages,
                    this.$page,
                    dismissedKey
